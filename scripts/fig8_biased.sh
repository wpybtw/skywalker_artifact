...
SP="--sage "
BATCH="--n 40000 "
LOG_FILE="./result/fig8_biased.csv"
...
echo "-------------------------------------------------------Skywalker offline sp 20 20 ${BATCH}" >>"${LOG_FILE}"
for idx in $(seq 1 ${#DATA[*]}); do
    for i in $(seq 1 ${ITR}); do
...
echo "-------------------Runtime of C-SAW  need to be scale by 10 due to 4k as batch size-------------------" >>"${LOG_FILE}"

echo "----------------------C-SAW  biased walk 4k 64-------------------" >>"${LOG_FILE}"
for idx in $(seq 1 ${#DATA[*]}); do
    echo "------------"${DATA[idx - 1]}
    /home/pywang/sampling/C-SAW/non-stream/sampling.bin wg ~/data/${DATA[idx - 1]}.w.edge_beg_pos.bin ~/data/${DATA[idx - 1]}.w.edge_csr.bin 100 32 4000 1 1 100 1 >>"${LOG_FILE}"
done

echo "----------------------C-SAW  sampling biased 4k 20 2 64-------------------" >>"${LOG_FILE}"
for idx in $(seq 1 ${#DATA[*]}); do
    echo "------------"${DATA[idx - 1]}
    /home/pywang/sampling/C-SAW/non-stream/sampling.bin wg ~/data/${DATA[idx - 1]}.w.edge_beg_pos.bin ~/data/${DATA[idx - 1]}.w.edge_csr.bin 100 32 4000 1 20 2 1 >>"${LOG_FILE}"
done
